...
for chi = chi_shear
  F_shear(:,chi==chi_shear) = ...
    ...
    - sigma .* shear_deformation - chi_shear(chi==chi_shear)^-1 * shear_deformation.^2;
...
effective_field_derivative = ...
  .26 * atan(.34 * (-3.1 + 2 * deformation)) ...
  - .53 * atan(.5 * (.45 + 2 * deformation)) ...
  + .03 * log(4.54 - 3.1 * deformation + deformation.^2) ...
  + .98 * log(.98 + .45 * deformation + deformation.^2) ...
  ;
...
effective_field_derivative = -.5 * ( ...
  4e2 * log(-2e1 + deformation) +  ...
  2 * log(-1 + deformation) + ...
  2 * log(1 + deformation) + ...
  1e5 * log(6e3 + deformation) ...
  );
effective_field_derivative = real(effective_field_derivative);
...
if is_dimensionless
  effective_field = effective_field * sqrt(alpha * theta);
end

shear_deformation = sqrt(theta) * ...
  (exp(effective_field.^2 / (2*alpha*theta)) * sqrt(2*pi) .* effective_field - ...
  pi * sqrt(alpha*theta) * erfi(effective_field / sqrt(2*alpha*theta))) ./ ...
  (pi * sqrt(alpha) * effective_field .* erfi(effective_field / sqrt(2*alpha*theta)));

if is_dimensionless
  shear_deformation = shear_deformation * sqrt(alpha) / sqrt(theta);
end
...
if is_dimensionless
  effective_field = effective_field * theta;
end

orientation_deformation = csch(3/2 * effective_field/theta) .* ...
  (3 * effective_field .* cosh(3/2 * effective_field/theta) - ...
  2 * theta * sinh(3/2 * effective_field/theta)) ./ (2 * effective_field);
