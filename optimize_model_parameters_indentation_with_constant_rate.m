% parameters = [...
%   8.704747856517300e+03 ...
%   1.220152675006576e+03 ...
%   7.154971140489589e-04 ...
%   0.125779602552493 ...
%   6.468083232289583e-25];
parameters = parameters_set(:,4);
options = optimset('Display','iter','PlotFcns',@optimplotfval);
fval = 1;


get_strains_rmse(parameters);i = i +1;
% while fval > .3
%   [parameters,fval] = fminsearch(@get_strains_rmse,parameters,options);
% end


%%
function [strains_rmse] = get_strains_rmse(parameters)

% indentation_force = readmatrix('c:\Users\User\Downloads\Красный.txt');
% indentation_force = indentation_force(7:end,:);
% indentation = indentation_force(:,1) * 1e-9;
% force = (indentation_force(:,2) ) * 1e-9;
% loading_time = 2 * 4e-2; % 1 - нагрузка, 2 - нагрузка-разгрузка
% time = (indentation / indentation(end)) * loading_time / 2;
% % time = [time; (indentation / indentation(end)) * loading_time / 2 + loading_time / 2];

% indentation_force = readmatrix('c:\Users\User\Downloads\Синий.txt');
% indentation = indentation_force(9:end,1) * 1e-9;
% force = (indentation_force(:,2) + 1 * abs(min(indentation_force(:,2)))) * 1e-9; %  
% force = force(9:end);
% loading_time = 2 * 17.5e-2; % 1 - нагрузка, 2 - нагрузка-разгрузка
% time = (indentation / indentation(end)) * loading_time / 2;
% % time = [time; (indentation / indentation(end)) * loading_time / 2 + loading_time / 2];
% 
% indentation_force = readmatrix('c:\Users\User\Downloads\зеленый.txt');
% indentation = indentation_force(14:end,1) * 1e-9;
% force = (indentation_force(:,2) + 1 * abs(min(indentation_force(:,2)))) * 1e-9; %  
% force = force(14:end);
% loading_time = 2 * 76e-2; % 1 - нагрузка, 2 - нагрузка-разгрузка
% time = (indentation / indentation(end)) * loading_time / 2;
% % time = [time; (indentation / indentation(end)) * loading_time / 2 + loading_time / 2];
% 
indentation_force = readmatrix('c:\Users\User\Downloads\желтый.txt');
indentation = indentation_force(9:end,1) * 1e-9;
force = (indentation_force(:,2) + 1 * abs(min(indentation_force(:,2)))) * 1e-9; %  
force = force(9:end);
loading_time = 2 * 370e-2; % 1 - нагрузка, 2 - нагрузка-разгрузка
time = (indentation / indentation(end)) * loading_time / 2;
% time = [time; (indentation / indentation(end)) * loading_time / 2 + loading_time / 2];

indentation_rate = indentation(end) / time(end);
radius = 2e-6;

E = 1e-9;
tau = 8e-1; % 1e-1 1e-1 1e-1 1e-1
alpha = .1;
beta = .1; % .0 .1 .2 .3
tip_geometry = 3/2;
F = E * gamma(tip_geometry + 1) * (time / time(end)).^tip_geometry ...
  .* ( 1 / gamma(tip_geometry + 1 - alpha) * (time / tau).^-alpha ...
      + 0 ); % 1 / gamma(tip_geometry + 1 - beta) * (time / tau).^-beta
...
nu = tau_1 * E1;
lambda_stat = parameters(4);
gamma_stat = parameters(5);

[~,fit_model] = get_xi_approximation;

sigma_0 = force(1) ./ (pi * (radius * indentation(1)));
initial_conditions = sigma_0;

[~,sigma] = ode15s(@(t,sigma) get_rhs_expr(t,sigma,indentation_rate,E0,E1,nu,theta,lambda_stat,gamma_stat,fit_model,radius,loading_time),time,initial_conditions);

color = 'm';
figure(2);
plot(time(:),force(1:end) ./ (pi * (radius * indentation(1:end))),'LineStyle','-','LineWidth',2);
hold on;
plot((time(:)),sigma,'LineStyle','-','LineWidth',2);
% plot(time,F)
hold off;
xlabel('{\itt}, с');ylabel('{\it\sigma}, Па')
...
F_orientation = ...
effective_field_integral_orientation_case ...
- gamma_stat / theta * (sigma(end) - E0 * indentation_rate * time(end)) .* orientation_deformation ...
- (theta / (lambda_stat * gamma_stat))^-1 * orientation_deformation.^2 / 2;

figure(4);hold on
plot(orientation_deformation,F_orientation - mean(F_orientation),color)
xlabel('{\itε}_{\ito}');ylabel('\Psi')

F_orientation = ...
effective_field_integral_orientation_case ...
- gamma_stat / theta * (sigma - E0 * indentation_rate * time) * orientation_deformation ...
- (theta / (lambda_stat * gamma_stat))^-1 * orientation_deformation.^2 / 2;
for i = 1:length(time)
  F_orientation_barrier(i) = get_energy_barrier(F_orientation(i,:));
end

% figure(5);hold on
% plot(time,nu * exp(F_orientation_barrier),color)
% xlabel('{\itt}, с');ylabel('\nu, кПа{\cdot}с')

figure(6);hold on
plot(time,F_orientation_barrier,color)
xlabel('{\itt}, с');ylabel('\Delta\Psi')

strains_rmse = sqrt( sum( ...
  abs(sigma - force(1:end) ./ (pi * (radius * indentation(1:end)))).^2 ...
  ./ abs(force(1:end) ./ (pi * (radius * indentation(1:end)))).^2 ) );
% strains_rmse = max(abs(sigma  - force(1:end) ./ (pi * (radius * indentation(1:end)))) ./ (force(1:end) ./ (pi * (radius * indentation(1:end)))) * 100);

end

function [rhs_expr] = get_rhs_expr(t,varibles,indentation_rate,E0,E1,nu,theta,lambda,gamma,fit_model,radius,loading_time)
...
strain = .2 * sqrt(indentation_rate * abs(t - heaviside(t - .501 * loading_time) * loading_time) / radius); % abs(t - heaviside(t - .501 * loading_time) * loading_time)

rhs_expr = zeros(size(varibles));
% sigma_t = (.04 / (pi * varibles(1).^2)) * loading_rate * t;
...
F_orientation = ...
  effective_field_integral_orientation_case ...
  - gamma / theta * (varibles(1) - E0 * strain) .* orientation_deformation ...
...
% dF_deps_o = 2/3 * theta / gamma * ...
%   (-20 * (8e3 / (-9e4 + eps_o) + 8e-2 / (-1 + eps_o) + 9e-2  / (6e-1 + eps_o)))...
%   - lambda * eps_o;
rhs_expr(1) = ...
  E1 / (nu * exp(F_orientation_barrier)) ... %  
  * ( .1 * sqrt(indentation_rate / (radius * abs(t - heaviside(t - .501 * loading_time) * loading_time))) * nu * exp(F_orientation_barrier) * (E0 + E1) / E1 ... %  * exp(F_orientation_barrier)
  + E0 * strain ...
...
% figure(3); hold on;
% plot(loading_rate * time(2:end),eps - ( loading_rate * time(2:end) - E0 * eps ) / E1,color)
% xlabel('{\it\sigma}, КПа');ylabel('{\itε}_{\ito}')
% 
% orientation_deformation = -.3:.01:.8;
% effective_field_integral_orientation_case = ...
% get_effective_field_integral_orientation_case(orientation_deformation);
...
% F_orientation = ...
% effective_field_integral_orientation_case ...
% - gamma_stat / theta * (loading_rate * time(2:end) - E0 * eps) * orientation_deformation ...
% - (theta / (lambda_stat * gamma_stat))^-1 * orientation_deformation.^2 / 2;
% for i = 1:4095
%   F_orientation_barrier(i) = get_energy_barrier(F_orientation(i,:));
% end
% 
% figure(5);hold on
% plot(time(2:end),nu * exp(F_orientation_barrier),color)
% xlabel('{\itt}, с');ylabel('\nu, кПа{\cdot}с')
% 
% figure(6);hold on
% plot(time(2:end),F_orientation_barrier,color)
% xlabel('{\itt}, с');ylabel('\Delta\Psi')
