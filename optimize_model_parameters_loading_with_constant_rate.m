parameters = [...
  2.249720085568877 ...
  9.958287026936057e+03 ...
  7.108258480731083e-06 ...
  0.002557168783561 ...
  3.609721167087118e-21];
options = optimset('Display','iter','PlotFcns',@optimplotfval);
fval = 1;


eps(i) = get_strains_rmse(parameters);i = i +1;
% while fval > .05
...
tau = 1e-0;
E = 1e0;
alpha = .937; % .71 .78 .85 .937
beta = .2;
lambda = 3/2;
loading_rate = 1e-3; % 1e-0 1e-1 1e-2 1e-3 
viscosity_1 = E * (tau)^alpha;
viscosity_2 = E * (tau)^beta;

time = linspace(0,5e-2,2^12)';

indentation_depth = (...
  loading_rate / viscosity_1 ...
  * time.^(1 + alpha) ...
  .* ml(viscosity_2 * time.^(alpha-beta) / viscosity_1,alpha - beta,2 + alpha) ...
  ).^(1 / lambda);
eps_kvf = .2 * indentation_depth(2:end).^.5;
...
[~,eps] = ode15s(@(t,eps) get_rhs_expr(t,eps,loading_rate,E0,E1,nu,theta,lambda_stat,gamma_stat,fit_model),time(2:end),initial_conditions);

color = 'r';
figure(2);
plot(time(2:end),eps_kvf,color,'LineStyle','-','LineWidth',2);
hold on;
plot(time(2:end),eps,color,'LineStyle','-','LineWidth',2);
hold off;
xlabel('{\itt}, с');ylabel('{\itε}')


strains_rmse = sqrt( sum( ...
  abs(eps - eps_kvf).^2 ...
  ./ abs(eps_kvf).^2 ) );
% strains_rmse = max(abs(eps - eps_kvf) ./ eps_kvf * 100);
...
%   - lambda * eps_o;
rhs_expr(1) = ...
  ( nu* exp(F_orientation_barrier)   / E1 * (.04 / (pi * varibles(1).^2)) * loading_rate ...
  + sigma_t ...
  - E0 * varibles(1) - dF_deps_o ) ...
  / ...
  ( (nu * exp(F_orientation_barrier)  * (E0 + E1)) / E1  + nu * exp(F_orientation_barrier) / E1 * .08 / (pi * varibles(1).^3) * loading_rate * t );
...
% F_orientation = ...
% effective_field_integral_orientation_case ...
% - gamma_stat / theta * (loading_rate * time(end) - E0 * eps(end)) .* orientation_deformation ...
